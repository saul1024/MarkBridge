...
- 一条命令将 HTML 导入指定 Chrome / Edge Profile。
...
- `--dry-run`：只预览输入 HTML、目标 Profile 和目标文件夹，不写浏览器。
...
- `--dry-run` 显示输入 HTML、目标 Profile 和目标文件夹，但不写浏览器。
- 正式执行会创建备份。
