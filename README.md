...
- `--dry-run` 显示来源 Profile、命中文件夹和输出路径，但不写 HTML。
- 正式执行后 `~/Desktop/books.html` 存在。
- 输出 HTML 只包含 `Books` 文件夹及其子树。

### 3. 预览并一键导入 Chrome

```sh
markbridge import-browser --input ~/Desktop/books.html --browser chrome --profile "Default" --folder "MarkBridge Test" --mode merge --dry-run
markbridge import-browser --input ~/Desktop/books.html --browser chrome --profile "Default" --folder "MarkBridge Test" --mode merge --quit-browser --reopen
```

预期：

- `--dry-run` 显示输入 HTML、目标 Profile、目标文件夹、预计新增书签和重复书签，但不写浏览器。
- 正式执行会创建备份。
- Chrome 打开后，在 `chrome://bookmarks` 可以看到 `Bookmarks Bar / MarkBridge Test`。
...
- 第二次正式导入不会在 `MarkBridge Test` 下生成重复书签。
- 如果没有任何新增内容，命令不会再创建无意义备份。

### 5. 替换目标目录但不影响其他目录

```sh
markbridge import-browser --input ~/Desktop/books.html --browser chrome --profile "Default" --folder "MarkBridge Test" --mode replace-folder --dry-run
markbridge import-browser --input ~/Desktop/books.html --browser chrome --profile "Default" --folder "MarkBridge Test" --mode replace-folder --quit-browser --reopen
```

预期：

- `Bookmarks Bar / MarkBridge Test` 会被当前 HTML 重建。
