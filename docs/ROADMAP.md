...
- `markbridge web`：仅绑定 127.0.0.1 的本机网页，用于选择 Chrome / Edge Profile 和文件夹。
- 跨 Profile 复制（主场景）：预览后显式执行，不经过 COS。
- COS 同步：显示 setup 状态，预览 push / pull，冲突时需勾选 force。
- 写浏览器操作默认预览；COS 密钥不下发到浏览器。

尚未完成：

- Electron / 桌面封装（当前明确不做）。
- 浏览器扩展（当前明确不做）。
- 完整 TUI。
- 可视化备份恢复与更完整的导入导出流程。
